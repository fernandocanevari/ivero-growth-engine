...
const FAQSection = () => {
  return (
    <section id="faq" className="py-16 bg-ivero-dark relative overflow-hidden">
      <div className="absolute top-0 right-1/4 w-96 h-96 bg-ivero-purple/8 rounded-full blur-3xl" />
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          <h2 className="font-display text-3xl md:text-4xl font-bold mb-4">
            <span className="text-primary-foreground">Perguntas </span>
          ...
          <Accordion type="single" collapsible className="space-y-3">
            {faqs.map((faq, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className="bg-ivero-dark-surface border border-ivero-purple/20 rounded-xl px-6 data-[state=open]:border-accent/30 data-[state=open]:shadow-[0_4px_20px_hsl(330_85%_55%/0.08)] transition-all duration-300"
              >
                <AccordionTrigger className="text-left font-semibold text-primary-foreground hover:no-underline py-5 gap-3">
                ...
                <AccordionContent className="text-ivero-slate-light leading-relaxed pb-5 pl-11">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
